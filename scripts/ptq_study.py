...
def load_ablation_natives() -> dict:
    ...
    Fails closed when source artifacts are missing.
    """
    variants = {
        ...
        for seed in SEEDS:
            ...
            if fp.exists():
                ...
                raise FileNotFoundError(
                    f"Missing native ablation artifact required for PTQ comparison: {fp}"
                )
    return out
...
def _native_variant_for_bits(bits: int):
    ...
    # bits=3 (Q3) and bits=0 (ternary) have no counterpart in the original
    # three-seed ablation matrix loaded by this script. Later single-seed native
    # campaign results are intentionally not mixed into this legacy comparison.
    ...
    with open(agg_path, "w") as f:
        json.dump({
            ...
            "caveats": {
                ...
                "q3_no_native": "No Q3 counterpart exists in the original three-seed native ablation matrix; a later single-seed native Q3 run is reported separately.",
                "ternary_no_native": "No ternary counterpart exists in the original three-seed native ablation matrix; a later single-seed native ternary run is reported separately.",
            },
        }, f, indent=2)
